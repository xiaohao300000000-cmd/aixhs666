...
| 稳定生产入口 | TODO | 固定后端地址、飞书入口、角色权限和审计 |

发布态：`https://tiho2o4ymck.aiforce.cloud/app/app_17a4790srtt`。当前可见范围为指定范围且要求飞书登录；线上尚未配置稳定 FastAPI 地址，因此默认展示明确降级态与无虚假数据的操作台结构预览。

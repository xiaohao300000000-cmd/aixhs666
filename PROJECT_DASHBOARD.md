...
| 当前主任务 | 用旧数据继续验证真实 DeepSeek、资格判断、飞书审核和话术审批；暂不做新的小红书采集 |
| 执行会话 | 主控单会话 |
| 当前分支 | `main` |
| 阻塞数量 | 4 |
| 最后更新 | 2026-07-09：功能分支已合并到 main；飞书话术审批按钮改为只写 approved_to_send，不在回调中直接发送小红书；全量测试 297 passed, 4 skipped |
...
| 日期 | 任务 | 结果 | 报告 |
|---|---|---|---|
| 2026-07-09 | 主线整理与话术发送降风险 | `feat/v15-agent-neutral-runtime` 已合并推送到 `main`；飞书话术审批按钮改为 `approved_to_send`，不再在回调线程里直接发送小红书；完整测试 `297 passed, 4 skipped, 1 warning` | `HANDOFF.md` |
...
1. 暂不做新采集，继续用旧数据验证 DeepSeek、Campaign、飞书审核和话术审批状态流转。
2. 为 `approved_to_send` 增加受控发送入口或 worker；浏览器/网络问题解决前不要真实触发小红书发送。
3. 在 `待人工确认卡片` 视图审核 61 个候选客户，标记 `可跟进` 或 `已忽略`。
4. 新建“客户审核卡片表”或调整主字段，让卡片标题直接显示 `需求摘要`。
5. 实现 `feishu-ai-review-sync`，让新增采集数据可增量进入 AI 筛选表。

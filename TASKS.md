...
- [ ] V19-05 打通公开回复草稿、两步确认、真实发送、三端状态与结果不确定恢复。（IN_PROGRESS：独立执行对话 `019f6be2-1325-7691-9774-477f7d95f212`，分支 `codex/v19-05-public-reply`，worktree `/Users/xiaohao30000/.codex/worktrees/f09f/aixhs666`）
- [ ] V19-06 实现 14:00 自动发现、15:00/21:00 回复检查、浏览器互斥调度和客户新回复提醒。
- [ ] V19-07 完成真实 PostgreSQL、Base、飞书、妙搭、Windows 浏览器和受控小红书目标的端到端及真人体验验收。

设计规格：`docs/superpowers/specs/2026-07-16-unified-operations-crm-loop-design.md`。V19 必须按 V19-01 至 V19-07 顺序形成可独立验收的切片；前一切片未通过自动化和规定的真实验收时，不进入依赖它的后一切片。

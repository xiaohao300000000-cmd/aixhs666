...
## 2026-07-16 V19 统一运营闭环设计确认

- 用户确认妙搭作为主操作台、飞书多维表格作为个人 CRM、飞书消息作为提醒和快捷入口，PostgreSQL/业务后端继续作为唯一事实源。
- 用户确认线索审核动作收敛为“推进为客户、暂缓判断、淘汰线索”；推进后必须进入客户、Base CRM、首次跟进和公开回复草稿，不再只修改状态后从队列消失。
- 用户确认公开回复采用“自动生成草稿 → 人工确认 → 点击真实发送”，第一阶段先打通公开回复，私信保留渠道接口。
- 用户确认每天 14:00 自动发现客户，15:00 和 21:00 检查客户新回复；21:00 发现回复立即飞书提醒并允许“现在处理”或“明天提醒我”。
...
## 2026-07-17 V19-04 妙搭产品闭环已验收发布

- 独立执行分支 `codex/v19-04-miaoda-operations` 经主控复验后合并到 `main`，合并提交为 `98c5c2a`；后续线上发现空/完成审核队列的继续按钮被错误禁用，已由全新独立执行对话完成热修并以 `422b768` 合并。
- 主控 fresh verification：后端全量 `594 passed, 7 skipped`；妙搭最终 Jest `40 passed`，TypeScript 类型检查、ESLint、Stylelint、生产构建和 `git diff --check` 均通过。
- 运营网关已通过 launchd 重启为 PID `67636`；本机与 Tailscale 公网健康检查均为 HTTP 200，Run `#8` 报告/候选、`2026-07-16` 每日队列和客户列表的鉴权 GET 均为 200。
- 首次正式发布为 release `7663164028453293028`、提交 `4821dca`；热修正式发布为 release `7663168275572280280`、提交 `a156df2`，两次状态均为 `finished`，线上入口保持 `https://tiho2o4ymck.feishuapp.com/app/app_17a4790srtt`。
- 已登录线上只读验收覆盖今日工作台、Run `#8` 任务报告、Run `#8` 的 50 条候选、客户列表、客户 `#147` 详情/时间线和系统健康；当前日空队列显示可用的“继续审核 20 条”和“只看高优先级”，Run 历史队列不错误显示继续按钮，页面无横向溢出。

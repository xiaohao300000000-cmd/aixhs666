...
同一个 Skill Run 从参数填写到完成结果持续更新同一条飞书消息。卡片点击回调必须在 3 秒内返回飞书规定的 `{}`、`toast` 或 `toast + card(type=raw)`；任务中心使用 `toast + raw Card 2.0` 立即替换当前卡片，不返回自定义 `code/msg/accepted` 包装。Worker 后续使用飞书“更新应用发送的消息”接口按 `message_id` PATCH 完整卡片内容。
...
- 回调在短事务中持久化 Skill Run。
- 直接返回 `toast + card(type=raw)`，把当前卡片改为参数表单。
...
- 记录 `queued` 事件。
- 回调快速返回飞书标准 `toast + card(type=raw)`，不得调用 DeepSeek、Campaign 执行或完整同步。

重复点击只返回当前持久化状态，不创建第二个 Skill Run 或第二个 CollectionTask。

Card 2.0 普通按钮的动作名来自 `event.action.value.action`，表单提交可来自 `event.action.name`；回调解析器必须兼容两种结构。配置 `FEISHU_ENCRYPT_KEY` 时，签名按 `SHA256(timestamp + nonce + encrypt_key + raw_body)` 校验，并解密外层 `encrypt` 字段后再做 token、幂等和业务处理。
...
## 10. 结果详情与同步可见性补充（2026-07-15）

- “查看结果”是独立产品状态，不等同于刷新完成摘要卡；结果卡必须展示参数、处理统计、同步状态和 Base 入口。
- `feishu_sync.dry_run > 0` 时必须明确提示“未写入多维表格”，不得仅显示新增/更新/失败均为 0。
- 正式运营配置下，Worker 在 `sync_feishu` 阶段自动写入 AI 筛选客户线索和证据明细；回调线程仍只持久化动作并快速返回。
- PostgreSQL 的 `feishu_bitable_records` 保存本地实体与远端 record ID 映射，作为幂等更新和异常恢复依据。

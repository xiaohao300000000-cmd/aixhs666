...
`status`（API 对外规范值；括号内为旧库兼容值）：

- `awaiting_approval`（`pending_review`）：等待人工审核或编辑。
- `approved`：当前版本已确认，但没有创建发送任务。
- `queued`（`approved_to_send`）：已完成第二步最终确认，并有唯一持久发送任务。
...
保存联系写命令的幂等结果：`operation_scope + entity_id + idempotency_key_hash` 唯一。相同键和相同请求重放原结果；相同键但请求不同明确拒绝。原始幂等键不落库。草稿编辑、版本确认、最终排队、平台结果和人工确认未发送均通过该表形成可重放事实。

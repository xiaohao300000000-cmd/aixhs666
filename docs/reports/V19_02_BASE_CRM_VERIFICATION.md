...
550 passed, 7 skipped, 1 warning in 25.86s
```

补充回归：

```text
CRM、旧客户跟进和 Base 同步组合回归：59 passed
CRM、Operator 与 lark-cli 元数据回归：27 passed
git diff --check：通过
```
...
- 抽样 10 个既有 `qualified` Lead 均回填为稳定 CRM 阶段，初始同步版本为 0。
- 真实客户 `customer_id=147` 用于闭环验收；未改变其资格状态，未生成发送任务。
- 最终 PostgreSQL：`customer_id=147`、`crm_stage=qualified`、`crm_sync_version=2`、`operator_note=null`。
...
| AI筛选证据明细 | 123 行 | 未变化 |
| 系统控制台 | 1 行 | 未变化 |
| 跟进记录 | 0 行、15 字段、3 视图 | 当前验收客户无既有跟进事实 |

没有删除任何字段、表、视图或记录；写前已有的 10 条客户记录及候选、证据、控制记录均未修改，没有覆盖未知数据。
...
该真实客户没有 `CustomerFollowupRecord`，因此两次同步后真实跟进表仍为 0 行；自动化测试另外证明“一个跟进事实一行”重复同步会复用相同映射，不产生重复跟进记录。
...
- 最终三端一致：客户 ID 147、阶段 `qualified/已确认客户`、同步版本 2、备注 `null`、同步状态 `synced`，Base record ID `recvpxkvVZRcv6`。
...
- 真实验收客户是迁移前已有客户，没有真实跟进事实；跟进记录真实表结构和空表已验证，非空投影的幂等性由自动化测试覆盖。

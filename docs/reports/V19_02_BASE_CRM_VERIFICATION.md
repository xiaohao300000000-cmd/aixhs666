...
- 主控复验发现 push/list 已隔离 candidate，但白名单 pull 仍只检查 Lead 是否存在。新增 watch/deferred candidate 劫持测试先得到 `customers_synced=1`；最小修复后顶层 `status=skipped`、`skipped=1`，且 Lead 字段、mapping、时间线和跟进事实均保持不变。
...
558 passed, 7 skipped, 1 warning in 26.31s
```
...
450d425 docs: finalize V19-02 verification evidence
bfb4401 fix: reject candidate rows during CRM pull
```
